...
exports.applyNodeOp = applyNodeOp;
const utils_1 = require("./utils");
function descendantIds(p, id) {
    const acc = [];
    const walk = (i) => { for (const c of p.nodes[i]?.childIds ?? []) {
        acc.push(c);
        walk(c);
    } };
    walk(id);
 ...
 */
function touchNode(p, id, now = new Date().toISOString()) {
...
function nextRev(p) {
    let max = 0;
    for (const n of Object.values(p.nodes))
        if (typeof n.rev === 'number' && n.rev > max)
            max = n.rev;
    return max + 1;
}
function newNode(id, type, title, parentId, rev, now) {
    return {
 ...
 */
async function applyNodeOp(p, op, io) {
    ...
    switch (op.type) {
        ...
        case 'move': {
            ...
            if (!node || op.id === op.newParentId)
                break;
            if (op.newParentId != null && descendantIds(p, op.id).includes(op.newParentId))
                break;
            if (node.parentId == null)
                p.rootIds = p.rootIds.filter((x) => x !== op.id);
            else {
                const old = p.nodes[node.parentId];
                if (old) {
            ...
            if (op.newParentId == null)
                p.rootIds.splice(op.atIndex, 0, op.id);
            else {
        ...
        case 'duplicate': {
            ...
            if (src.parentId == null) {
                const i = p.rootIds.indexOf(op.id);
                p.rootIds.splice(i + 1, 0, newId);
            }
            else {
        ...
        case 'trash': {
            const node = p.nodes[op.id];
            if (!node || !p.trashId || node.parentId === p.trashId)
                break;
            if (node.parentId == null)
                p.rootIds = p.rootIds.filter((x) => x !== op.id);
            else {
        ...
        case 'updateMeta':
            if (p.nodes[op.id]) {
                p.nodes[op.id].meta = { ...p.nodes[op.id].meta, ...op.patch };
                touch(op.id);
            }
            break;
        case 'setExpanded':
